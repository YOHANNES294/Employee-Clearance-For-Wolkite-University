...
const DropdownUser = () => {
  ...
  const [userData, setUserData] = useState(null); // Changed to null instead of []
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      if (!Id) return; // Avoid fetching if Id isn’t available yet
      try {
        ...
        setUserData(fetchedData); // Assuming fetchedData is a single object
      } catch (error) {
    ...
    fetchUserData();
  }, [Id]); // Removed userData from dependencies to avoid infinite loop
  ...
  // Prevent rendering until session and userData are available
    ...
    <div className="relative">
      ...
      >
        <span className="hidden text-right lg:block">
          ...
          <span className="block text-xs">
            {userData?.privilege ? userData.privilege : session?.user?.role}
          </span>
        </span>
      ...
      >
        <ul className="flex flex-col gap-5 border-b border-stroke px-6 py-7.5 dark:border-strokedark">
          ...
          <li>
            <div className="flex items-left gap-3.5 text-sm font-medium duration-300 ease-in-out hover:text-primary lg:text-base">
              <DarkModeSwitcher />
            </div>
          </li>
        </ul>
